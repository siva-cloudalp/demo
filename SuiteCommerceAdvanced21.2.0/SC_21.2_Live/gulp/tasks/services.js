...
gulp.task('services', function(cb) {
    ...
    const arr_services_controllers = [];
    _.each(
        package_manager.getContents(),
        function(module) {
            if (module.content['autogenerated-services']) {
                _.each(
                    module.content['autogenerated-services'],
                    function(service) {
                        arr_services_controllers.push(
                            `${module.baseDir}/SuiteScript/${service}.js`
                        );
                    },
                    this
                );
    ...
    _.each(arr_services_controllers, function(controller) {
        try {
            ...
            console.log(
                `Warning: the file ${controller} is required by a service but couldn't be found`
            );
		}
	});
    ...
    const servicesConfig = package_manager.getAutogeneratedServices();
        ...
        _.each(obj, function(value, key) {
            ...
            if (!obj[key]) {
                console.log(
                    'Service generation',
                    key,
                    ' skipped. Controller name must be specified as a value'
                );
			}
            if (key.indexOf('/') >= 0 || key.indexOf('\\') >= 0) {
                console.log(
                    'Service generation',
                    key,
                    ' skipped. Please specify only the filename as the key in the ns.package.json'
                );
            } else if (key.indexOf('*') >= 0) {
                console.log(
                    'Service generation',
                    key,
                    ' skipped. Wildcards are not allowed in the service name'
                );
            } else {
                mkdirSync(path.join(process.gulp_dest, 'services'), { recursive: true });

                const dest = path.join(process.gulp_dest, 'services', key);
				...
				*/
                ...
                const serviceCode = template({ controllerName: obj[key] });
				fs.writeFileSync(dest, serviceCode);
			}
		});
	});

    const arrAutogeneratedServices = _.flatten(
        _.map(servicesConfig, function(service) {
    ...
    const globs = package_manager.getGlobsFor('services');

    _.each(globs, function(pathToFiles) {
        const arrFiles = glob.sync(pathToFiles);
        _.each(arrFiles, function(pathToFile) {
            const serviceName = path.basename(pathToFile);
            if (_.indexOf(arrAutogeneratedServices, serviceName) !== -1) {
                console.warn(
                    `Warning: ${serviceName} will not be autogenerated because there is a custom service with the same name`
                );
			}
		});
	});
	...
	.pipe(package_manager.handleOverrides())
        .pipe(gulp.dest(path.join(process.gulp_dest, 'services'), { mode: 0o700 }));
});
