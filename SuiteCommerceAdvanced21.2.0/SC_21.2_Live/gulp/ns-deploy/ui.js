...
const ui = {
        ...
        if (deploy.info.authID) {
            ...
            if (!tokenList.length) {
                log(
                    colorText(
                        color.YELLOW,
                        `Remember to have TOKEN-BASED AUTHENTICATION feature enabled in the target account.`
                    )
                );
            }
    ...
    getWebsitesAndDomains: function(deploy, cb) {
        if (deploy.info.website && deploy.info.domain) {
            ...
            cb(
                new Error(
                    'Wrong response. Are you sure the selected account has SuiteCommerce Lite bundle installed ?'
                )
            );
        } else {
            inquirer
                ...
                .then(function(answers) {
                    ...
                    const domains = _.filter(deploy.websitesAndDomains.websiteDomainData, function(
                        data
                    ) {
                    ...
                    inquirer
                        ...
                        .then(function(answers) {
                            ...
                            const domainInfo = _.find(
                                deploy.websitesAndDomains.websiteDomainData,
                                function(data) {
                                    return (
                                        data.domain === deploy.info.domain &&
    ...
    rollback: function(deploy, cb) {
        if (deploy.rollback_revision) {
            ...
            let longest = 0;
            const choices = deploy.revisions
                .sort(function(a, b) {
                    return b.deployment_time - a.deployment_time;
                })
                .map(function(revision) {
                    let name = `${new Date(revision.deployment_time)
                        .toISOString()
                        .slice(0, 19)
                        .replace('T', ' ')}`;
    ...
    targetSSPFolder: folderInquirer({
        ...
        choices: function(deploy) {
            let applications = deploy.folder_inquirer.target_publisher_folder.publisher.application;
