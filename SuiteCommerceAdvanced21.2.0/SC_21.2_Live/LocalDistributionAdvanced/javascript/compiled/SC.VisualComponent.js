...
*/
define("SC.VisualComponent", ["require", "exports", "underscore", "jQuery", "SC.BaseComponent.ChildViewsComponent", "ExportedModulesNames", "GlobalViews.Message.View", "SC.BaseComponent", "Backbone.View.render"], function (require, exports, _, jQuery, SC_BaseComponent_ChildViewsComponent_1, ExportedModulesNames_1, GlobalViews_Message_View_1, SC_BaseComponent_1) {
    "use strict";
    // @module SC
    var NotificationsTypes;
    (function (NotificationsTypes) {
        NotificationsTypes["ERROR"] = "error";
        NotificationsTypes["WARNING"] = "warning";
        NotificationsTypes["SUCCESS"] = "success";
        NotificationsTypes["INFO"] = "info";
    ...
    var visual_component = _.extend({}, SC_BaseComponent_1.SCBaseComponent, {
        ...
        extend: function extend(child_component) {
            ...
            new_component.application
                ...
                .on('beforeAppendView', function (view) { return new_component._onApplicationBeforeView(view); });
            new_component.application
                .getLayout()
                .on('afterAppendView', function (view) { return new_component._onApplicationAfterAppendView(view); });
        ...
        _isViewComponent: function _isViewComponent(viewIdentifier) {
            var componentIdentifiers = this._getComponentIdentifiers();
            var isComponent = false;
            _.each(componentIdentifiers, function (componentIdentifier) {
        ...
        setChildViewIndex: function setChildViewIndex(view_id, placeholder_selector, view_name, 
        ...
        _setChildViewIndex: function _setChildViewIndex(viewId, placeholderSelector, viewName, index) {
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                if (!this._isViewComponent(viewId)) {
                    console.warn('INVALID_PARAM', "Invoking the method \"setChildViewIndex\" with an incorrect parameter: (" + viewId + ").\n\t\t\t\tIt will still work but it will be deprecated in the future. Please check the documentation.");
                }
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.setChildViewIndex(viewId, placeholderSelector, viewName, index);
            }
            else {
                return this._reportError('INVALID_PARAM', "The specified viewId (" + viewId + ") is not valid for the current component.");
            }
        },
        ...
        registerView: function registerView(dataView, viewConstructor) {
            var generator = {};
            generator[dataView] = {};
        ...
        addContextData: function (viewId, name, func) {
            ...
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.addContextData(viewId, name, func);
            }
            else {
                return this._reportError('INVALID_PARAM', "The specified viewId (" + viewId + ") is not valid for the current component.");
            }
        ...
        _addChildViews: function _addChildViews(viewId, childViews) {
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                if (!this._isViewComponent(viewId)) {
                    console.warn('INVALID_PARAM', "Invoking the method \"addChildViews\" with an incorrect parameter: (" + viewId + ").\n\t\t\t\tIt will still work but it will be deprecated in the future. Please check the documentation.");
                }
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.addChildViews(viewId, childViews);
            }
            else {
                return this._reportError('INVALID_PARAM', "The specified viewId (" + viewId + ") is not valid for the current component.");
            }
        ...
        _removeChildView: function removeChildView(viewId, placeholderSelector, viewName) {
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                if (!this._isViewComponent(viewId)) {
                    console.warn('INVALID_PARAM', "Invoking the method \"removeChildView\" with an incorrect parameter: (" + viewId + ").\n\t\t\t\tIt will still work but it will be deprecated in the future. Please check the documentation.");
                }
                viewName = viewName || placeholderSelector;
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.removeChildView(viewId, placeholderSelector, viewName);
            }
            else {
                return this._reportError('INVALID_PARAM', "The specified viewId (" + viewId + ") is not valid for the current component.");
            }
        },
        isDataTypeValid: function (dataType) {
        ...
        // @return {null} null if everything works as expected. An exception will be thrown otherwise.
        addToViewContextDefinition: function addToViewContextDefinition(viewId, propertyName, type, callback) {
            if (!_.isFunction(callback)) {
                this._reportError('INVALID_PARAM', 'The specified callback is not valid.');
            }
            if (!this.isDataTypeValid(type)) {
                this._reportError('INVALID_DATA_TYPE', 'Invalid data type. Please check the json-schema documentation for valid data types.');
            }
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                ...
                if (this._currentAddContextProperty[viewId][propertyName]) {
                    this._reportError('DUPLICATED_CONTEXT_PROPERTY', 'Duplicated propertyName. Trying to add more than one extra context property with the same name.');
                }
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.addToViewContextDefinition(viewId, propertyName, type, callback);
        ...
        // @param {String} property_name Name of the new property to be added
        // @return {null} null if everything works as expected. An exception will be thrown otherwise.
        removeToViewContextDefinition: function removeToViewContextDefinition(viewId, propertyName) {
            if (!(this._currentAddContextProperty[viewId] &&
                this._currentAddContextProperty[viewId][propertyName])) {
                this._reportError('INVALID_PARAM', 'The specified extra context property does not exists or is not custom.');
            }
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.removeToViewContextDefinition(viewId, propertyName);
        ...
        // @return {null} null if everything works as expected. An exception will be thrown otherwise.
        modifyViewJsonLd: function modifyViewJsonLd(viewId, callback) {
            if (!_.isFunction(callback)) {
                this._reportError('INVALID_PARAM', 'The specified callback is not valid.');
            }
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.modifyViewJsonLd(viewId, callback);
            }
            else {
                return this._reportError('INVALID_PARAM', "The specified viewId (" + viewId + ") is not valid for the current component.");
            }
        },
        isEventSelectorValid: function (eventSelector) {
            var eventName = [
                'blur',
                'change',
                'click',
        ...
        // @return {Void}
        addToViewEventsDefinition: function addToViewEventsDefinition(viewId, eventSelector, callback) {
            if (!_.isFunction(callback)) {
                this._reportError('INVALID_PARAM', 'The specified callback is not valid.');
            }
            if (!this.isEventSelectorValid(eventSelector)) {
                this._reportError('INVALID_PARAM', 'The specified eventSelector parameter does not respect the required format.');
            }
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                ...
                if (this._currentAddEventHandler[viewId][eventSelector]) {
                    this._reportError('DUPLICATED_CONTEXT_PROPERTY', 'Duplicated eventSelector. Trying to add more than one extra context property with the same name.');
                }
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.addToViewEventsDefinition(viewId, eventSelector, callback);
        ...
        // @param {String} event_selector
        // @return {Void}
        removeToViewEventsDefinition: function removeToViewEventsDefinition(viewId, eventSelector) {
            if (!(this._currentAddEventHandler[viewId] &&
                this._currentAddEventHandler[viewId][eventSelector])) {
                this._reportError('INVALID_PARAM', 'The specified event selector does not exists or is not custom.');
            }
            if (ExportedModulesNames_1.isModuleLoaded(viewId)) {
                SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.removeToViewEventsDefinition(viewId, eventSelector);
                this._currentAddEventHandler[viewId][eventSelector] = true;
            }
            else {
                return this._reportError('INVALID_PARAM', "The specified viewId (" + viewId + ") is not valid for the current component.");
        ...
        showMessage: function showMessage(options) {
            ...
            if (!(options.type.toUpperCase() in NotificationsTypes)) {
                return this._reportError('INVALID_PARAM', "The specified notification type (" + options.type + ") is not valid, the availables types are error, warning, info or success.'");
            }
            options.selector = options.selector || 'Notifications';
            var selectedDiv = jQuery("[data-view=\"" + options.selector + "\"], [data-cms-area=\"" + options.selector + "\"]");
            if (selectedDiv.length === 0) {
                return this._reportError('INVALID_PARAM', "The specified selector (" + options.selector + ") does not exist.'");
            }
            ...
            var message_id = _.uniqueId(options.type + "_message_");
            global_view_message.$el.attr('id', message_id);
            selectedDiv.append(global_view_message.render().$el);
            if (options.timeout) {
                setTimeout(function () {
                    _this.closeMessage(message_id);
                }, options.timeout);
            }
            return message_id;
        },
        closeMessage: function closeMessage(message_id) {
            var message = jQuery("#" + message_id);
            if (!message_id || !message) {
                return this._reportError('INVALID_PARAM', "The specified message id (" + message_id + ") is not valid'");
            }
...
//# sourceMappingURL=SC.VisualComponent.js.map
