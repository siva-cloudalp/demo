...
*/
define("Cart.Lines.Free.View", ["require", "exports", "underscore", "cart_lines_free.tpl", "Configuration", "GlobalViews.Message.View", "LiveOrder.Model", "Transaction.Line.Views.Price.View", "Transaction.Line.Views.Options.Selected.View", "ProductLine.Stock.View", "ProductLine.Sku.View", "ProductLine.StockDescription.View", "Transaction.Line.Views.Tax", "Backbone.View"], function (require, exports, _, cart_lines_free_tpl, Configuration_1, GlobalViews_Message_View_1, LiveOrderModel, TransactionLineViewsPriceView, TransactionLineViewsOptionsSelectedView, ProductLineStockView, ProductLineSkuView, ProductLineStockDescriptionView, TransactionLineViewsTax, BackboneView) {
    "use strict";
    return BackboneView.extend({
        ...
        addQuantity: function () {
            ...
            var new_val = Math.min(this.model.get('free_gift_info').qtyelegible, parseFloat(old_value) + 1);
        ...
        subQuantity: function () {
            var old_value = this.$('[name="quantity"]').val();
            var new_val = Math.max(1, parseFloat(old_value) - 1);
            this.$('[name="quantity"]').val(new_val);
            this.changeQuantity();
        },
        removeGift: function () {
        ...
        updateGift: function (quantity) {
            ...
            var line = this.model;
            var current_quantity = line.get('quantity');
            line.set('quantity', quantity);
            var invalid = line.validate();
            var placeholder = this.$el.find('[data-type="alert-placeholder"]');
            placeholder.empty();
            if (!invalid) {
        ...
        changeQuantity: _.debounce(function () {
            ...
            var current_quantity = parseInt(this.model.get('quantity'), 10);
            var new_quantity = $input.val() <= this.model.get('free_gift_info').eligible_quantity && $input.val() > 0
                ? parseInt($input.val())
                : current_quantity;
...
//# sourceMappingURL=Cart.Lines.Free.View.js.map
