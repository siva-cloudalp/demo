...
*/
define("Checkout.Component", ["require", "exports", "underscore", "Utils", "jQuery", "SC.VisualComponent", "OrderWizard.Model"], function (require, exports, _, Utils, jQuery, SCVisualComponent, OrderWizardModel) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.CheckoutComponent = void 0;
    var is_wizard_router_ready = jQuery.Deferred();
    var order_wizard_model = OrderWizardModel.getInstance();
    ...
    var CheckoutComponent = function CheckoutComponentGenerator(application) {
        ...
        var api_methods = {
            ...
            _normalizeModule: function _normalizeModule(module) {
                var options = Utils.deepCopy(module.module[1]);
                var module_id = options.module_id;
                var module_index = options.module_index;
            ...
            _normalizeStepGroup: function _normalizeStepGroup(inner_step_group) {
                ...
                var commonStepGroupAttrs = ['index', 'name', 'url', 'show_group', 'state', 'steps'];
            ...
            setCurrentStep: function setCurrentStep(data) {
                ...
                try {
                    ...
                    var stepsOrder_1 = self.order_wizard_router.stepsOrder;
                    var steps_1 = self.order_wizard_router.steps;
                    var current_step_url = self.order_wizard_router.currentStep;
                    var current_step_index_1 = _.indexOf(stepsOrder_1, current_step_url);
                    var target_step_index_1 = _.indexOf(stepsOrder_1, data.step_url);
                    if (_.isEmpty(current_step_url)) {
                        this._reportError('INVALID_STATE', 'Invalid state. There is no current step yet');
                    }
                    if (target_step_index_1 < 0) {
                    ...
                    self.cancelableTrigger('beforeSetCurrentStep', data.step_url)
                        ...
                        if (current_step_index_1 < target_step_index_1) {
                            for (var i = current_step_index_1; i < target_step_index_1; i++) {
                                step_url = stepsOrder_1[i];
                                step = steps_1[step_url];
                                if (!step.showStep()) {
                                    continue;
                                }
                                queue_submits(step);
                        ...
                        else if (current_step_index_1 > target_step_index_1) {
                            for (var j = current_step_index_1; j > target_step_index_1; j--) {
                                step_url = stepsOrder_1[j];
                                step = steps_1[step_url];
                                if (!step.showStep()) {
                                    continue;
                                }
            ...
            getStepsInfo: function getStepsInfo() {
                ...
                var deferred = jQuery.Deferred();
                try {
                    var steps_2 = self.order_wizard_router.steps;
                    var steps_order = self.order_wizard_router.stepsOrder;
                    deferred.resolve(_.map(steps_order, function (step_url) {
                        var step = steps_2[step_url];
            ...
            removeModuleFromStep: function removeModuleFromStep(data) {
                ...
                try {
                    ...
                    var step_1 = this._getStep(data.step_url);
                    var module_1 = _.findWhere(step_1.moduleInstances, {
                        module_id: data.module_id
                    });
                    var module_index_1 = _.indexOf(step_1.moduleInstances, module_1);
                    self.cancelableTrigger('beforeRemoveModuleFromStep', data)
                        .done(function () {
                        step_1.moduleInstances.splice(module_index_1, 1);
                        module_1.destroy();
                        var current_view = self.application.getLayout().getCurrentView();
                        current_view && current_view.render();
            ...
            removeStepsGroup: function removeStepsGroup(data) {
                ...
                try {
                    ...
                    var steps_groups_1 = self.order_wizard_router.stepGroups;
                    var steps_group = steps_groups_1[data.group_name];
                    if (!steps_group) {
                    ...
                    if (steps_group.steps.length) {
                        self._reportError('INVALID_PARAM', 'Invalid parameter. The step group (' +
                            data.group_name +
                            ') cannot be removed because it has steps');
                    }
                    self.cancelableTrigger('beforeRemoveStepsGroup', data)
                        .done(function () {
                        delete steps_groups_1[data.group_name];
                        var groups = _.values(steps_groups_1);
            ...
            removeStep: function removeStep(data) {
                ...
                try {
                    ...
                    var steps_3 = self.order_wizard_router.steps;
                    var step_2 = steps_3[data.step_url];
                    if (!step_2) {
                    ...
                    self.cancelableTrigger('beforeRemoveStep', data)
                        ...
                        step_2.stepGroup.steps = _.reject(step_2.stepGroup.steps, function (step_group_step) {
                            return step_group_step === step_2;
                        });
                        delete steps_3[data.step_url];
