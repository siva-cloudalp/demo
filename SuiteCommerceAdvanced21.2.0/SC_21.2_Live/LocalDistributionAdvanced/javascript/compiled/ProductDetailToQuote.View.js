...
*/
define("ProductDetailToQuote.View", ["require", "exports", "underscore", "product_detail_to_quote.tpl", "Utils", "Configuration", "GlobalViews.Message.View", "Profile.Model", "ProductList.Model", "ProductList.Item.Model", "Session", "Backbone.View"], function (require, exports, _, productDetailToQuote_tpl, Utils, Configuration_1, GlobalViews_Message_View_1, Profile_Model_1, ProductListModel, ProductListItemModel, Session, BackboneView) {
    "use strict";
    // @class ProductDetailToQuote.View @extend Backbone.View
    var ProductDetailToQuoteView = BackboneView.extend({
        ...
        initialize: function initialize(options) {
            ...
            this.profile_promise = Profile_Model_1.ProfileModel.getPromise();
            this.state = {
                feedbackMessage: '',
        ...
        attachOnProfileModelEvents: function attachOnProfileModelEvents() {
            ...
            this.profile_promise.done(function user_profile_done_loading() {
                self.profile_model = Profile_Model_1.ProfileModel.getInstance();
        ...
        itemToQuote: function itemToQuote(e) {
            ...
            if (this.profile_model.get('isLoggedIn') === 'T' && !this.state.quote_permissions) {
                var phone = Configuration_1.Configuration.get('quote.defaultPhone', '');
                var email = Configuration_1.Configuration.get('quote.defaultEmail', '');
                this.state.feedbackMessageType = 'warning';
                this.state.feedbackMessage = Utils.translate("Sorry, you don't have sufficient permissions to request a quote online. <br/> For immediate assistance <strong>call us at $(0)</strong> or email us to <strong>$(1)</strong>", phone, email);
                this.render();
            }
                ...
                this.application.ProductListModule.Utils.getRequestAQuoteProductList().done(function (product_list_json) {
                    ...
                    if (!product_list_json.internalid) {
                        ...
                        var product_list_line_json_1 = product_list_line.toJSON();
                        var item_present_in_list = _.find(product_list_json.items, function (product_list_line_aux) {
                            return (parseInt(product_list_line_aux.item.internalid) ===
                                product_list_line_json_1.item.internalid &&
                                _.isEqual(product_list_line_json_1.options, product_list_line_aux.options));
                        });
        ...
        addItemToQuote: function (product_list, product_list_line, product) {
            var quantity_to_add;
            var self = this;
            var show_enforced_quantity_message = false;
            ...
            product_list_line
                ...
                if (show_enforced_quantity_message) {
                    self.state.feedbackMessage = Utils.translate('$(0) has been added to your <a href="#" data-hashtag="#request-a-quote" data-touchpoint="customercenter" data-trigger="go-to-quote">Quote Request</a><br>Quantity: $(1). (Enforced minimum quantity)', product_name, quantity_to_add);
                }
                else {
                    self.state.feedbackMessage = Utils.translate('$(0) has been added to your <a href="#" data-hashtag="#request-a-quote" data-touchpoint="customercenter" data-trigger="go-to-quote">Quote Request</a><br>Quantity: $(1)', product_name, quantity_to_add);
                }
                self.render();
            });
        },
        ...
        updateItemInQuote: function (item_in_list) {
            var self = this;
            var quantity_to_add = this.model.get('quantity');
            var new_quantity = parseInt(item_in_list.quantity, 10) + parseInt(quantity_to_add, 10);
            var product_list_item_model = new ProductListItemModel({
        ...
        isQuotable: function () {
            return (!(this.model.get('item') &&
                (this.model.get('item').get('itemtype') === 'GiftCert' ||
                    this.model.get('item').get('itemtype') === 'Discount')) && this.application.ProductListModule.Utils.isProductListEnabled());
        },
    ...
    return ProductDetailToQuoteView;
});

//# sourceMappingURL=ProductDetailToQuote.View.js.map
