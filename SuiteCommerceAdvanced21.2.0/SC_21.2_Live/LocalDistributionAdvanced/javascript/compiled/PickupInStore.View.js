...
*/
define("PickupInStore.View", ["require", "exports", "underscore", "pickup_in_store.tpl", "Utils", "Profile.Model", "PickupInStore.StoreSelector.View", "Location.VenueDetails.View", "LiveOrder.Model", "ReferenceMap", "Location", "Backbone.View", "js.cookie"], function (require, exports, _, pickup_in_store_tpl, Utils, Profile_Model_1, PickupInStoreStoreSelectorView, LocationVenueDetailsView, LiveOrderModel, ReferenceMap, Location, BackboneView, Cookies) {
    "use strict";
    var PickupInStoreView = BackboneView.extend({
        ...
        initialize: function initialize(options) {
            ...
            var location_id = this.model.get('location').get('internalid') || Cookies.get('myStore');
            if (location_id) {
        ...
        changeStore: function changeStore(e) {
            e.preventDefault();
            var self = this;
            var view = new PickupInStoreStoreSelectorView({
                model: this.model,
        ...
        getContext: function getContext() {
            ...
            var stock_information = this.model.getStockInfo();
            var location_stock = this.getLocationStock();
            var is_available_for_ship = item.get('_isBackorderable') || item.get('_isInStock');
            var internal_id = this.source && this.source === 'cart'
            ...
            return {
                ...
                getDirectionsUrl: location && !location.isNew()
                    ? this.reference_map.getDirectionsUrl(this.profile_model.get('storeLocator_last_search'), location.get('location'))
                    : '',
    ...
    return PickupInStoreView;
});

//# sourceMappingURL=PickupInStore.View.js.map
