...
*/
define("QuickAdd.View", ["require", "exports", "underscore", "quick_add.tpl", "quick_add_item.tpl", "Utils", "Configuration", "ItemsSearcher.View", "QuickAdd.Model", "QuickAdd.Item.View", "QuickAdd.ItemsSearcher.Plugins", "Transaction.Line.Views.Options.Selected.View", "ProductViews.Price.View", "ProductLine.Stock.View", "ProductLine.Sku.View", "Transaction.Line.Model", "Backbone", "Backbone.FormView", "Backbone.View"], function (require, exports, _, quick_add_tpl, quick_add_item_tpl, Utils, Configuration_1, ItemsSearcherView, QuickAddModel, QuickAddItemView, QuickAddItemsSearcherPlugins, TransactionLineViewsOptionsSelectedView, ProductViewsPriceView, ProductLineStockView, ProductLineSkuView, TransactionLineModel, Backbone, BackboneFormView, BackboneView) {
    "use strict";
    // @class QuickAdd.View @extends Backbone.View
    var QuickAddView = BackboneView.extend({
        ...
        setQuantity: function (e) {
            if (this.model.get('selectedProduct')) {
                ...
                var quantity = parseInt(str_quantity, 10);
                var minimum_quantity = this.model
                    .get('selectedProduct')
                    .get('item')
                    .get('_minimumQuantity', true);
                var maximum_quantity = this.model
                    .get('selectedProduct')
                    .get('item')
                    .get('_maximumQuantity', true);
                var quantity_already_set = 0;
                if (_.isFunction(this.options.getItemQuantitySet)) {
                    quantity_already_set =
                        this.options.getItemQuantitySet(this.model.get('selectedProduct').getItemId()) || 0;
                }
        ...
        subQuantity: function (e) {
            ...
            var old_value = quantity_input.val();
            var new_val = parseFloat(old_value) - 1;
            new_val = Math.max(this.model
                .get('selectedProduct')
                .get('item')
                .get('_minimumQuantity', true), new_val);
            quantity_input.val(new_val);
            quantity_input.change();
        },
        ...
        onKeyDown: function (e) {
            var jq_event = event;
            var key_code = e.which ? e.which : jq_event.keyCode;
            if (key_code > 31 &&
        ...
        onItemSelected: function (result) {
            ...
            if (product) {
                ...
                if (minimum_quantity > 1) {
                    this.$('[data-type="minimum-quantity"]').html(Utils.translate('Minimum of $(0) required', minimum_quantity));
                }
        ...
        getContext: function () {
            ...
            return {
                ...
                itemTitle: this.options.itemTitle || Utils.translate('Which item(s) would you like to add?'),
...
//# sourceMappingURL=QuickAdd.View.js.map
