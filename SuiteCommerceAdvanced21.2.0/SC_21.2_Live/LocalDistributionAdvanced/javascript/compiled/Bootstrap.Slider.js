...
*/
define("Bootstrap.Slider", ["require", "exports", "jQuery"], function (require, exports, $) {
    "use strict";
    ...
    Slider.prototype = {
        ...
        handleMouseMove: function (e) {
            ...
            var page_x = this.getPageX(e);
            var slid_value = this.getSlidValue(page_x);
            var is_low = this.$dragging.data('control') === 'low';
            var value = Math[is_low ? 'min' : 'max'](slid_value, this.values[is_low ? 'high' : 'low']);
        ...
        getPageX: function (e) {
            var touch = e.originalEvent && e.originalEvent.touches
        ...
        getSlidValue: function (page_x) {
            ...
            var maxBoundary = this.getMaxBoundary();
            var location = page_x > maxBoundary ? maxBoundary : page_x < minBoundary ? minBoundary : page_x;
            return (((location - minBoundary) / this.$element.innerWidth()) * this.getSizeInValue() +
                this.values.min);
        },
        getClosestControl: function (page_x) {
            var value = this.getSlidValue(page_x);
            var distance_low = Math.abs(this.values.low - value);
            var distance_high = Math.abs(this.values.high - value);
            var $low = this.controls.$low;
