...
define('ProductList.Item.Model', [
    ...
    return SCModel.extend({
        ...
        get: function(user, id) {
            ...
            const sort_column = 'custrecord_ns_pl_pli_item';
            const sort_direction = 'ASC';
            const productlist_items = Search.searchHelper(
                filters,
                sort_column,
                sort_direction,
                true
        ...
        delete: function(user, id) {
            ...
            const product_list_item = nlapiLoadRecord('customrecord_ns_pl_productlistitem', id);
            const parent_product_list = ProductList.get(
                user,
                product_list_item.getFieldValue('custrecord_ns_pl_pli_productlist')
            );
        ...
        create: function(user, data) {
            ...
            const parent_product_list = ProductList.get(user, data.productList.id);

            if (parseInt(parent_product_list.owner.id, 10) !== user) {
                throw unauthorizedError;
            }

            const productListItem = nlapiCreateRecord('customrecord_ns_pl_productlistitem');

            data.description &&
                productListItem.setFieldValue(
            ...
            if (data.options) {
                data.options &&
                    productListItem.setFieldValue(
                        'custrecord_ns_pl_pli_options',
                        JSON.stringify(this.parseLineOptionsToProductList(data.options || {}))
                    );
            }
        ...
        update: function(user, id, data) {
            ...
            product_list_item.setFieldValue(
                'custrecord_ns_pl_pli_description',
                this.sanitize(data.description)
            );
            data.options &&
