    ...
    return {
        ...
        search: function(user, product_list_id, include_store_item, sort_and_paging_data) {
            ...
            const filters = [
                new nlobjSearchFilter(
                    'custrecord_ns_pl_pli_productlist',
                    null,
                    'is',
                    product_list_id
                ),
            ...
            let sort_column = sort_and_paging_data.sort;
            let sort_direction = sort_and_paging_data.order;
            ...
            let search_lines = this.searchHelper(
                filters,
                sort_column,
                sort_direction === '-1' ? 'DESC' : 'ASC',
                include_store_item
            );

            if (include_store_item && sort_column === 'price') {
                // -1 for descending, 1 for ascending
                search_lines = this.sortLinesByPrice(
                    search_lines,
        ...
        sortLinesByPrice: function(lines, sort_direction) {
            return _.sortBy(lines, function(line) {
                ...
                const price_detail = line.item.onlinecustomerprice_detail || {};
                let price = price_detail.onlinecustomerprice || line.item.pricelevel1 || 0;
                const { quantity } = line;

                if (quantity && price_detail.priceschedule && price_detail.priceschedule.length) {
                    const price_schedule = _.find(price_detail.priceschedule, function(
                        price_schedule
                    ) {
                        return (
        ...
        searchHelper: function(filters, sort_column, sort_direction, include_store_item) {
            // Selects the columns
            const productListItemColumns = {
                internalid: new nlobjSearchColumn('internalid'),
                name: new nlobjSearchColumn('formulatext', 'custrecord_ns_pl_pli_item').setFormula(
                    'case when LENGTH({custrecord_ns_pl_pli_item.displayname}) > 0 then {custrecord_ns_pl_pli_item.displayname} else {custrecord_ns_pl_pli_item.itemid} end'
                ),
                sku: new nlobjSearchColumn('formulatext', 'custrecord_ns_pl_pli_item').setFormula(
                    '{custrecord_ns_pl_pli_item.itemid}'
                ),
                ...
                priority_value: new nlobjSearchColumn(
                    'custrecord_ns_pl_plip_value',
                    'custrecord_ns_pl_pli_priority'
                ),
            ...
            const records = Application.getAllSearchResults(
                ...
                filters,
                _.values(productListItemColumns)
            );
            const productlist_items = [];
            const self = this;

            _(records).each(function(productListItemSearchRecord) {
                const itemInternalId = productListItemSearchRecord.getValue(
                    'custrecord_ns_pl_pli_item'
                );
                const itemId = productListItemSearchRecord.getText('custrecord_ns_pl_pli_item');
                const itemMatrixParent = productListItemSearchRecord.getValue(
                    'parent',
                    'custrecord_ns_pl_pli_item'
                );
                const created_date = nlapiStringToDate(
                    productListItemSearchRecord.getValue('created'),
                    window.dateformat
                );
                const created_date_str = nlapiDateToString(created_date, window.dateformat);
                const itemType = productListItemSearchRecord.getValue(
                    'type',
                    'custrecord_ns_pl_pli_item'
                );
                const productListItem = {
                    internalid: productListItemSearchRecord.getId(),
                    description: productListItemSearchRecord.getValue(
                        'custrecord_ns_pl_pli_description'
                    ),
                    options: self.parseLineOptionsFromProductList(
                        JSON.parse(
                            productListItemSearchRecord.getValue('custrecord_ns_pl_pli_options') ||
                                '{}'
                        )
                    ),
                    quantity: parseInt(
                        productListItemSearchRecord.getValue('custrecord_ns_pl_pli_quantity'),
                        10
                    ),
            ...
            const store_item_references = _(productlist_items).pluck('store_item_reference');
            const results = [];
            ...
            _(productlist_items).each(function(productlist_item) {
                const { store_item_reference } = productlist_item;
                ...
                if (include_store_item || !StoreItem) {
                    ...
                    productlist_item.item.internalid = parseInt(
                        productlist_item.item.internalid,
                        10
                    );
                } else {
