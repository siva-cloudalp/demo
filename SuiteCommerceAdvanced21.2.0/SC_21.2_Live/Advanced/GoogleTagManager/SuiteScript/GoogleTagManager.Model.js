...
define('GoogleTagManager.Model', ['SC.Model', 'Application', 'underscore'], function(
    SCModel,
    Application,
    _
) {
    return SCModel.extend({
        ...
        save: function(id, events) {
            ...
            const filters = [new nlobjSearchFilter('custrecord_ns_gtm_gid', null, 'is', id)];
            const columns = {
                internalid: new nlobjSearchColumn('internalid'),
                gid: new nlobjSearchColumn('custrecord_ns_gtm_gid'),
                events: new nlobjSearchColumn('custrecord_ns_gtm_events')
            };
            const gtm_datalayer = this.search(filters, columns);

            if (!gtm_datalayer) return this.create(id, events);

            let current_events = gtm_datalayer.getFieldValue('custrecord_ns_gtm_events');
            ...
            const extended_events = current_events.concat(events || []);

            gtm_datalayer.setFieldValue(
                'custrecord_ns_gtm_events',
                JSON.stringify(extended_events)
            );
        ...
        search: function(filters, columns) {
            ...
            const gtm_datalayer = Application.getAllSearchResults(
                'customrecord_ns_gtm_datalayer',
                filters,
                _.values(columns)
            );
        ...
        create: function(id, events) {
            const gtm_datalayer = nlapiCreateRecord('customrecord_ns_gtm_datalayer');
