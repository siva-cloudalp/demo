...
define('GoogleTagManager.ServiceController', [
    ...
    function save() {
        ...
        if (Configuration.get().tracking.googleTagManager.isMultiDomain) {
            if (
                Utils.recordTypeExists('customrecord_ns_gtm_datalayer') &&
                Utils.recordTypeHasField(
                    'customrecord_ns_gtm_datalayer',
                    'custrecord_ns_gtm_gid'
                ) &&
                Utils.recordTypeHasField(
                    'customrecord_ns_gtm_datalayer',
            ...
            nlapiLogExecution(
                'DEBUG',
                "[Only for multi-domain sites] Please check if the customrecord_ns_gtm_datalayer's custom record and its required fields has been created.-"
            );
            return {
                internalid: 1,
                gtm_gid: 1,
        ...
        const ctx = ModelsInit.context;
        const session_obj = ctx.getSessionObject('gtmDataLayer');
        let dataLayerSession;
    ...
    return ServiceController.extend({
        ...
        getDataLayer: function(request, response) {
            let googletagmanager_cookie = request.getParameter('_ga') || '';
            if (googletagmanager_cookie) {
                ...
                if (
                    ...
                    googletagmanager_cookie = googletagmanager_cookie.replace(
                        /.*?_gid=.*?\.(\d+\.\d+);.*/,
                        '$1'
                    );
                } else {
