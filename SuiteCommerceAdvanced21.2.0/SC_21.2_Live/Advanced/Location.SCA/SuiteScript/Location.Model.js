...
define('Location.Model', [
    ...
    return SCModel.extend({
        ...
        get: function(data) {
            ...
            const search_results = this.search(data);
        ...
        search: function(data) {
            ...
            if (this.data.locationtype) {
                const location_type = _.isArray(this.data.locationtype)
                    ? this.data.locationtype
                    : this.data.locationtype.split(',');

                this.filters.push(
                    new nlobjSearchFilter('locationtype', null, 'anyof', location_type)
                );
            }
            ...
            if (this.data.latitude && this.data.longitude) {
                ...
                const formula = this.getDistanceFormulates();
                if (this.data.radius) {
                    this.filters.push(
        ...
        getServiceHours: function(id) {
            const records = _.filter(this.searchServiceHoursResults || [], function(record) {
        ...
        getFriendlyName: function(id) {
            if (
                Configuration.get().storeLocator &&
        ...
        getDistanceFormulates: function() {
            ...
            const { PI } = Math;
            const R = Configuration.get().storeLocator.distanceUnit === 'mi' ? 3959 : 6371;
            const lat = (this.data.latitude * PI) / 180;
            const lon = (this.data.longitude * PI) / 180;
            const formula =
                R +
                ' * (2 * ATAN2(SQRT((SIN((' +
                lat +
        ...
        getRecordValues: function(record) {
            const map_result = {};
            const id = record.getValue('internalid');
            ...
            if (this.data.latitude && this.data.longitude) {
                const distance = Math.round(record.getValue('formulanumeric') * 10) / 10;
