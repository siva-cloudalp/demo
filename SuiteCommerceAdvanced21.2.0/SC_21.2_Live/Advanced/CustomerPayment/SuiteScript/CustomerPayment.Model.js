...
define('CustomerPayment.Model', ['Transaction.Model', 'ExternalPayment.Model', 'Utils'], function(
    TransactionModel,
    ExternalPayment,
    Utils
) {
    ...
    return TransactionModel.extend({
        ...
        getPaymentMethod: function() {
            ...
            const { paymentmethods } = this.result;
        ...
        getInvoices: function() {
            ...
            for (let i = 1; i <= this.record.getLineItemCount('apply'); i++) {
                const apply = this.record.getLineItemValue('apply', 'apply', i) === 'T';
                ...
                if (apply) {
                    const invoice = {
                        ...
                        total_formatted: Utils.formatCurrency(
                            this.record.getLineItemValue('apply', 'total', i),
                            this.currencySymbol
                        ),
                        apply: apply,
                        ...
                        disc_formatted: Utils.formatCurrency(
                            this.record.getLineItemValue('apply', 'disc', i),
                            this.currencySymbol
                        ),
                        amount: Utils.toCurrency(
                            this.record.getLineItemValue('apply', 'amount', i)
                        ),
                        ...
                        due_formatted: Utils.formatCurrency(
                            this.record.getLineItemValue('apply', 'due', i),
                            this.currencySymbol
                        ),
        ...
        getExtraRecordFields: function() {
            ...
            this.result.balance_formatted = Utils.formatCurrency(
                this.record.getFieldValue('balance'),
                this.currencySymbol
            );

            // @property {Number} payment_formatted
            this.result.payment_formatted = Utils.formatCurrency(
                this.record.getFieldValue('payment'),
                this.currencySymbol
            );
            ...
            if (this.record.getFieldValue('paymethtype') === 'external_checkout') {
                ...
                this.result.redirecturl = ExternalPayment.generateUrl(
                    this.result.internalid,
                    this.result.recordtype
                );

                // @property {String} paymenteventholdreason
                this.result.paymenteventholdreason = this.record.getFieldValue(
            ...
            this.result.invoices_total_formatted = Utils.formatCurrency(
                this.record.getFieldValue('applied'),
                this.currencySymbol
            );
        }
    });
