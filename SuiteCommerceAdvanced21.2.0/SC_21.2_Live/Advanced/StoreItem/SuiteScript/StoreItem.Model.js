...
define('StoreItem.Model', ['SC.Model', 'SC.Models.Init', 'underscore', 'Configuration'], function(
    SCModel,
    ModelsInit,
    ...
    return SCModel.extend({
        ...
        preloadItems: function(items, fieldset_name) {
            ...
            const items_by_id = {};
            const parents_by_id = {};
            const fieldKeys = Configuration.get('fieldKeys');
            const itemsFieldsStandardKeys = (fieldKeys && fieldKeys.itemsFieldsStandardKeys) || [];
            ...
            const items_details = this.getItemFieldValues(items_by_id, fieldset_name);
            ...
            if (_.size(parents_by_id)) {
                const parents_details = this.getItemFieldValues(parents_by_id, fieldset_name);
            ...
            _.each(this.preloadedItems, function(item) {
                if (item.itemoptions_detail && item.itemoptions_detail.matrixtype === 'child') {
                    item.matrix_parent = self.getPreloadedItem(
        ...
        getItemFieldValues: function(items_by_id, fieldset_name) {
            const item_ids = _.values(items_by_id);
            const is_advanced =
                ModelsInit.session.getSiteSettings(['sitetype']).sitetype === 'ADVANCED';
            ...
            if (is_advanced) {
                try {
                    ...
                    return ModelsInit.session.getItemFieldValues(
                        fieldset_name,
                        _.pluck(item_ids, 'internalid')
                    ).items;
                } catch (e) {
